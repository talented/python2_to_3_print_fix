...
def update_line(line):
    ...
    #    return ('{}({})').format(new_line.split()[0], " ".join(new_line.split()[1:]))
    #else:
    #    return new_line
...
def update_pre_block(pre_block):
    ...
    if "print" not in pre_block:
        return pre_block
    else:
        #return pre_block.split("\n")
...
def update_file(input_file_name, output_file_name):
    ...
    writefile.write(updated_text)

    writefile.close()

# A couple of test files
update_file("table.html", "table_updated.html")
update_file("CodeSkulptor Documentation.html", "CodeSkulptor Documentation_updated.html")
